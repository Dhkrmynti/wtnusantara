        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->editColumn('status', function($row) {
                    $color = $row->status === 'published' ? 'text-green-500 bg-green-500/10' : 'text-slate-500 bg-slate-500/10';
                ...
                ->addColumn('action', function($row) {
                    ...
                    $btn .= '                Edit Post';
                    $btn .= '            </button>';
                    $btn .= '            <form action="' . route('admin.posts.destroy', $row->id) . '" method="POST" onsubmit="return confirm(\'Hapus berita ini?\')">';
                    $btn .= '                ' . csrf_field();
