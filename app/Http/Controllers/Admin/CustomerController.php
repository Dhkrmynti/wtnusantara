...
class CustomerController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = Customer::with('package')->select('customers.*');
            return DataTables::of($data)
                ...
                ->addColumn('action', function($row){
                    ...
                    $btn .= '            <button @click="openDrawer(\'edit\', ' . htmlspecialchars(json_encode($row)) . '); open = false" class="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-secondary hover:bg-primary/5 hover:text-primary transition-all">';
                    ...
                    $btn .= '                Edit Pelanggan';
                    $btn .= '            </button>';
                    $btn .= '            <form action="' . route('admin.customers.destroy', $row->id) . '" method="POST" onsubmit="return confirm(\'Hapus pelanggan ini?\')">';
                    $btn .= '                ' . csrf_field();
                    $btn .= '                ' . method_field('DELETE');
                    $btn .= '                <button type="submit" class="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-red-500 hover:bg-red-500/5 transition-all">';
